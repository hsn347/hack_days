...
HISTORY_FILE = "exclude_history_Invaders.json"
...
def run_bot(bot):
    ...
    # استرجاع الأهداف السابقة المستبعدة من ملف JSON لهذا الحساب
    exclude_map = get_exclude_dict(MY_UID)
    if exclude_map:
        print(f" 📋 أهداف مستبعدة في الذاكرة ({len(exclude_map)}/10): {list(exclude_map.keys())}")
    ...
    if r2:
        print(f" near world 🥵🥵🥵  = {json.dumps(r2, ensure_ascii=False, indent=2)}")
        # حفظ الهدف الجديد في ملف الـ JSON
        if r2.get('result') and len(r2['result']) > 0 and 'id' in r2['result'][0]:
            found_id = r2['result'][0]['id']
            add_to_exclude_history(MY_UID, found_id)
            print(f" 💾 تم تسجيل الهدف {found_id} في ملف JSON لـ UID {MY_UID}")


    r3 = bot.query('1002', '7', {"uid": MY_UID})
    if r3:
        print(f" my map 🤢🤢🤢  = {json.dumps(r3, ensure_ascii=False, indent=2)}")
    ...
    if r4:
        d = r4.get('data', {})
        print(f" formation 👿👿👿 = {json.dumps(r4, ensure_ascii=False, indent=2)}")

    r5 = bot.query('1006', '15', {"x": r2['result'][0]['x'], "y": r2['result'][0]['y'],"kingdomId": r3['data']['base']['partition'] , "id": r2['result'][0]['id']})
    if r5:
        print(f" building info 🤣😂🤣 = {json.dumps(r5, ensure_ascii=False, indent=2)}")
    ...
    r6 = bot.query('1007', '2', {
        ...
        "heros": {},
        "matrixType": 3,
        "mapId": r3['data']['base']['partition'],
        "moveLineType": 3,
        "data": {
            "data": {
                "currentSourceNum": r5['retData']['resource']['currentSourceNum'],
                "resourceType": 1000 + Lineup
            },
            "to": {
                "y": r2['result'][0]['y'],
                "x": r2['result'][0]['x'],
