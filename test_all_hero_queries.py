...
def test():
    ...
    queries = [
        ("1005", "7", {"compiletype": 1}),
        ("1005", "1", {}),
        ("1005", "5", {}),
        ("2058", "21", {}),
    ]

    for cmd, sub, data in queries:
        responses.clear()
        script.post({'type': 'cmd', 'cmd': cmd, 'subcmd': sub, 'data': data})
        time.sleep(1.0)
        print(f"\n==================== {cmd}/{sub} ====================")
        for r in responses:
            print(f"[{cmd}/{sub}] -> {json.dumps(r, ensure_ascii=False, indent=2)}")
