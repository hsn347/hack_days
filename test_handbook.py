...
def test_handbook():
    ...
    print("[+] Connected! Testing Hero queries...")

    # 1. Test 2058/21 (Hero Handbook)
    responses.clear()
    script.post({'type': 'cmd', 'cmd': '2058', 'subcmd': '21', 'data': {}})
    time.sleep(1.2)
    for r in responses:
        print(f"2058/21 response: {json.dumps(r, ensure_ascii=False, indent=2)[:500]}")

    # 2. Test 3079/3 (War Gem Hall)
    responses.clear()
    script.post({'type': 'cmd', 'cmd': '3079', 'subcmd': '3', 'data': {}})
    time.sleep(1.2)
    for r in responses:
        print(f"3079/3 response: {json.dumps(r, ensure_ascii=False, indent=2)[:500]}")
